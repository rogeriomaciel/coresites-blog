...
Your goal is to fetch data from GA4, analyze it, and provide a 5-part report:
1. **O que tá bombando (Trends):** Top pages by views.
2. **De onde vem o tráfego (Sources):** Top traffic sources/mediums.
3. **Tráfego Real (Brazil Locations):** City breakdown for real human traffic in Brazil.
4. **Assunto mais relevante (Key Topic):** The common theme among the top performing pages, prioritizing Brazil data to filter out bot data center traffic.
5. **Ideias de Conteúdo (Content Ideas):** 3 to 5 new content titles/outlines based on the top topics.
...
### 3. Analyze the Data
The script will output a JSON containing `top_pages`, `top_sources`, and `brazil_locations`.
- Review the `top_pages` to see what is currently trending.
- Review the `top_sources` to understand where users are coming from.
- Review `brazil_locations` to isolate real human traffic (ignoring bot-heavy traffic from data centers in US/Europe).
- Analyze the page titles/paths to deduce the most relevant overarching topic based on real traffic.
...
**3. Tráfego Real (Brasil):**
- [City 1] - [Sessions] sessões
- [City 2] - [Sessions] sessões

**4. Assunto mais relevante:**
- [Your analysis of the common theme and why it's working. IMPORTANT: Use the Brazil locations to identify real human traffic and filter out bot data centers from the analysis.]

**5. O que podemos criar:**
